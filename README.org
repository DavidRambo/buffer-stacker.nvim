...
** Usage
   The provided installation snippet sets keymaps for all three commands plus the picker: ~BufstackerNext~, ~BufstackerPrev~, and ~BufstackerLs~.
   Open and close files as before and enjoy the intuitive ordering of buffers as you visit them.
   While a command for the picker is exposed, ~BufstackerTelescope~, the installation script calls it directly in order to pass config options.
